...
PLUGIN_AGENT_OVERVIEW_SECTIONS = [
    ...
    },
    {
        "title": "提示词格式建议与示例",
        "items": [
            "建议尽量按“**要做什么** + **挂在哪个 Hook** + **具体处理规则** + **是否需要配置项**”来描述，这样 Agent 更容易一次生成对的插件。",
            "如果是“替换词”“清洗文本”“统一格式”这类需求，最好直接写出几组明确例子，比如“把 A 替换成 B”“删除句尾的 C”“把全角标点统一成半角标点”。",
            "如果你想改已有插件，建议直接说明“修改哪个插件 + 想新增什么行为 + 不要改什么”，这样 Agent 会更稳地在单个插件目录内完成修改。",
            "推荐格式示例：**帮我新建一个挂在 after_translate 的普通翻译后处理插件，把译文里的‘老师’统一替换成‘导师’，把‘厉害’替换成‘真强’，并把替换词做成可配置项。**",
            "推荐格式示例：**帮我新建一个挂在 before_render 的渲染前插件，统一把字号下限设为 28，把描边打开，并把文字颜色强制改成深灰色。**",
            "推荐格式示例：**帮我修改现有的某某插件，保留原有逻辑，只额外在 after_ocr 阶段去掉 OCR 文本首尾空格，并把连续三个以上的句号压成三个点。**",
        ],
    },
]
