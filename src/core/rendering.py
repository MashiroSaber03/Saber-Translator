...
def compact_special_symbols(text: str) -> str:
    ...
    - 不再合并连续省略号，以保留原文的情感表达层次
      (例如: ...... 表示长时间沉默，不应被压缩为 ...)
    - 只删除全角标点后的空格（符合中日韩排版规范）
    - 保留半角标点后的空格（避免破坏英文可读性）
    ...
    # 【修复】只删除全角标点后的空格，保留半角标点后的空格
    # 原因：
    # 1. 全角标点本身包含留白，后面的空格是AI翻译的多余输出，应删除
    # 2. 半角标点后的空格是英文排版的必要组成部分，不应删除
    # 3. 避免出现 "What?No!" 这样的英文可读性问题
    pattern = r'([。，、！？；：…—～「」『』【】（）《》〈〉])[ 　]+'
