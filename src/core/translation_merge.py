    ...
    while retry_count < max_retries:
        try:
            if model_provider == 'siliconflow':
                ...
                chat_messages = None
                # 获取多轮对话缓存
                chat_messages = chat_cache.get(chat_session, None)
                if not chat_messages:
                    chat_messages = init_chat_cache()
                ...
                # 补全多轮对话缓存
                try:
                    json_check = json.loads(translated_text)['translated_text']
                    if chat_session:
                        chat_messages.append({"role": "assistant", "content": translated_text})
                        chat_cache[chat_session] = chat_messages
                except:
...
def translate_text_list(texts, target_language, model_provider,
                        api_key=None, model_name=None, prompt_content=None, chat_session=None,
                        use_json_format=False, custom_base_url=None,
                        rpm_limit_translation: int = constants.DEFAULT_rpm_TRANSLATION,
                        max_retries: int = constants.DEFAULT_TRANSLATION_MAX_RETRIES):
    """
    ...
    if model_provider.lower() == 'mock':
        ...
        if model_provider.lower() == 'deepseek':
            translated_json = translate_single_text(
            ...
            if isinstance(translated_json, str):
                translated_texts = json.loads(translated_json)['translated_text']
            else:
                translated_texts = translated_json
        # 正常翻译流程
        else:
            for i, text in enumerate(texts):
