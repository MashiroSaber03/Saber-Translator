...
import * as bubbleState from './bubble_state.js'; // 导入统一的气泡状态模块
...
export function translateImageApi(params) {
    // 从 state 中获取当前的 rpm 设置、重试次数和检测器类型并添加到参数中
    const apiParams = {
        ...params,
        rpm_limit_translation: state.rpmLimitTranslation,
        rpm_limit_ai_vision_ocr: state.rpmLimitAiVisionOcr,
        max_retries: state.translationMaxRetries,  // 重试次数
        detector_type: state.textDetector,  // 文本检测器类型
        // 文本框扩展参数
        box_expand_ratio: state.boxExpandRatio,
        box_expand_top: state.boxExpandTop,
        box_expand_bottom: state.boxExpandBottom,
        box_expand_left: state.boxExpandLeft,
        box_expand_right: state.boxExpandRight,
        // 精确文字掩膜
        usePreciseMask: state.usePreciseMask,
        maskDilateSize: state.maskDilateSize,
        maskBoxExpandRatio: state.maskBoxExpandRatio
    };
...
/**
 * 高质量翻译批量请求 - 通过后端代理调用 AI 服务，避免 CORS 问题
 * @param {object} params - 包含翻译参数的对象
 * @param {string} params.provider - AI 服务商 (siliconflow, deepseek, volcano, gemini, custom_openai)
 * @param {string} params.api_key - API 密钥
 * @param {string} params.model_name - 模型名称
 * @param {string} [params.custom_base_url] - 自定义服务的 Base URL
 * @param {Array} params.messages - OpenAI 格式的消息数组
 * @param {boolean} [params.low_reasoning] - 是否使用低推理模式
 * @param {boolean} [params.force_json_output] - 是否强制 JSON 输出
 * @param {string} [params.no_thinking_method] - 取消思考的方法 (gemini/volcano)
 * @returns {Promise<object>} - 包含 AI 响应内容的 Promise
 */
export function hqTranslateBatchApi(params) {
    return makeApiRequest('/api/hq_translate_batch', 'POST', params);
}
...
/**
 * 单气泡OCR识别
 * @param {object} params - 包含OCR参数的对象
 * @param {string} [params.image_data] - base64 图片数据（未裁剪，后端裁剪）
 * @param {string} [params.bubble_image] - 已裁剪气泡的 base64 图片数据（推荐）
 * @param {Array} params.bubble_coords - 气泡坐标 [x1, y1, x2, y2]
 * @param {string} params.ocr_engine - OCR引擎 (manga_ocr, paddle_ocr, baidu_ocr)
 * @param {string} [params.baidu_ocr_api_key] - 百度OCR API Key
 * @param {string} [params.baidu_ocr_secret_key] - 百度OCR Secret Key
 * @returns {Promise<object>} - 包含识别结果的 Promise
 */
export function ocrSingleBubbleApi(params) {
    return makeApiRequest('/api/ocr_single_bubble', 'POST', params);
}
...
/**
 * 对单个气泡区域进行背景修复（LAMA）
 * @param {object} params - 包含修复参数的对象
 * @param {string} params.image_data - base64 图片数据
 * @param {Array} params.bubble_coords - 气泡坐标 [x1, y1, x2, y2]
 * @param {string} [params.method='lama'] - 修复方法
 * @returns {Promise<object>} - 包含修复后图片数据的 Promise
 */
export function inpaintSingleBubbleApi(params) {
    return makeApiRequest('/api/inpaint_single_bubble', 'POST', params);
}
 ...
 * @returns {Promise<object>} - 包含会话数据的 Promise
 */
export function loadSessionByPathApi(sessionPath) {
    return makeApiRequest('/api/sessions/load_by_path', 'POST', { path: sessionPath });
}
 ...
 * @returns {Promise<object>} - 包含保存结果的 Promise
 */
export function saveChapterSessionApi(bookId, chapterId, sessionData) {
    return makeApiRequest('/api/bookshelf/save_chapter_session', 'POST', {
        book_id: bookId,
 ...
 * 分批保存 - 第一步：开始保存
 * @param {string} sessionPath - 会话路径
 * @param {object} metadata - 元数据（不含图片 Base64）
 */
export function batchSaveStartApi(sessionPath, metadata) {
    return makeApiRequest('/api/sessions/batch_save/start', 'POST', {
...
/**
 * 分批保存 - 第二步：保存单张图片
 * @param {string} sessionFolder - 会话文件夹路径（后端返回的绝对路径）
 * @param {number} imageIndex - 图片索引
 * @param {string} imageType - 图片类型 (original/translated/clean)
 * @param {string} imageData - Base64 图片数据
 */
export function batchSaveImageApi(sessionFolder, imageIndex, imageType, imageData) {
    return makeApiRequest('/api/sessions/batch_save/image', 'POST', {
...
/**
 * 分批保存 - 第三步：完成保存
 * @param {string} sessionFolder - 会话文件夹路径
 * @param {array} imagesMeta - 更新后的图片元数据
 */
export function batchSaveCompleteApi(sessionFolder, imagesMeta) {
    return makeApiRequest('/api/sessions/batch_save/complete', 'POST', {
        session_folder: sessionFolder,
