...
> 说明：
> 当前前端翻译主链内部已经有 `save` 原子步骤，但插件系统**暂未开放** `before_save / after_save` 钩子。
> 因此插件文档里列出的“可接入步骤”仍然只有上面这些。
...
`pipeline` 是一个特殊步骤，区别于上面 7 个可插件接入的原子步骤：
...
- **新建插件目录后，不必重启后端；在前端“插件管理”里点击“刷新插件”即可重新扫描**
- **修改插件源码后，不必重启后端；点击“刷新插件”会尝试热重载新的 Python 代码**
...
当前版本支持在前端「设置 -> 插件管理」中导出单个插件。

导出规则：

- 导出格式是单个 `.zip`
- 只包含插件源码和 `manifest.json`
- **不会**包含：
  - `config/plugin_configs/<plugin_id>.json`
  - `config/plugin_default_states.json`
  - `__pycache__`
  - `.pyc`

这意味着导出包更适合“分享插件代码”，而不是“完整迁移本机状态”。

导出包中的 `manifest.json` 主要用于：
...
- 导入后会先做结构校验，再尝试加载插件
- 新导入的插件默认**不会自动启用**
- 如果本地已存在相同 `plugin_id`，系统会提示你是否替换
...
也就是说，导入/替换更像是“升级插件代码”，而不是“重置你的本地配置”。
