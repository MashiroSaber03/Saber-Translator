...
打包版默认数据目录是 EXE 同级的 `data-v2`，与启动时的工作目录无关；源码版默认为项目下的 `data-v2`。只有显式传入 `--data-dir` 才会使用其他目录。更新到新的程序目录时，应在退出桌面和后端后保留并迁入整个 `data-v2`。程序不会自动读取或搬迁过去留在用户目录中的数据，也不再通过环境变量指定数据目录。
...
当前应用版本和存储版本以根目录 `version.json` 为准。保留已经发布的 `3.5.0 → 3.5.1` 字体目录转换，以及 `3.5.1 → 3.5.2` 无数据变更步骤，兼容已有数据；不回退或重编号历史存储版本。3.5.0 仍是首个基线，无版本旧数据不支持迁移。
