...
当前版本为 **3.5.2**，已经登记 `3.5.0 → 3.5.1` 字体目录转换，以及 `3.5.1 → 3.5.2` 无数据变更步骤。3.5.2 沿用 3.5.1 的数据库结构，只推进版本号。3.5.0 仍是首个基线，无版本旧数据不支持迁移。测试中的自定义转换只属于隔离样例。

1. 更新根 `version.json`，发行标签必须完全对应，例如 `v3.5.2`。CI 同时检查版本清单、标签和对应 SQL 契约文件，PyInstaller 打包版本清单与固定契约。
...
4. 结构和所有持久化内容都未变化时，例如显式声明 `Migration("3.5.1", "3.5.2")`；目标 DDL 应保持同样的结构。纯空迁移链校验通过后，在一个事务中仅推进最终版本，不做完整备份、不主动终止任务。
