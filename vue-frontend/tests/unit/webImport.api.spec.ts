...
describe('backend-owned web import diagnostics', () => {
  ...
  it('writes draft selections through the current typed command endpoint', async () => {
    putMock.mockResolvedValue({
      draftId: 'draft/id',
      revision: 3,
      selectedPageIds: ['page-1'],
    })
    const { updateWebImportSelection } = await import('@/api/v2/webImport')

    await updateWebImportSelection('draft/id', 2, ['page-1'])

    expect(putMock).toHaveBeenCalledWith(
      '/api/v2/web-import/drafts/draft%2Fid/selection',
      { baseRevision: 2, selectedPageIds: ['page-1'] },
  ...
  it('tests explicit Firecrawl and agent credentials through v2 diagnostics', async () => {
    const { testAgentConnection, testFirecrawlConnection } = await import('@/api/v2/webImport')

    await testFirecrawlConnection('firecrawl-key')
    await testAgentConnection('deepseek', 'agent-key', 'https://agent.example/v1', 'deepseek-chat')
