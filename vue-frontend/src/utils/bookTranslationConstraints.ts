...
function normalizeGlossaryEntries(
  value: unknown
): BookTranslationConstraints['glossary']['entries'] {
...
function normalizeNonTranslateEntries(
  value: unknown
): BookTranslationConstraints['non_translate']['entries'] {
  if (!Array.isArray(value)) {
    return []
  }
...
export function normalizeBookTranslationConstraints(payload?: unknown): BookTranslationConstraints {
  ...
  return {
    glossary: {
      ...
      autoExtractPrompt:
        readString(glossaryPayload.autoExtractPrompt).trim() || DEFAULT_AUTO_GLOSSARY_PROMPT,
      entries: glossaryEntries.length > 0 ? glossaryEntries : defaults.glossary.entries,
    },
    non_translate: {
      enabled: Boolean(nonTranslatePayload.enabled),
      entries:
        nonTranslateEntries.length > 0 ? nonTranslateEntries : defaults.non_translate.entries,
    },
  }
}
